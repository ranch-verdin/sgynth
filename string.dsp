...
myString(res, off, offcomp) = excitation : nl_lagrangecombfilter(maxDel,N,aN,off)
with{
	...
	N = ma.SR/freq;
	b0 = 1;
	aN = ba.tau2pole(res * freq * 0.00001)*(off * offcomp + 1.0);
...
process = (inargs : vgroup("string1", myString)),
	...
	offcomp = hslider("offcomp",0.0, 0.0, 0.1, 0.001);
	off = hslider("off",0.0, 0.0, 10.0, 0.01);
	inargs = res, offcomp, off;
};
